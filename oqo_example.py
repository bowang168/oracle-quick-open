...
Quickly open Oracle SR / KM docs / Bug / JIRA / Schedule / Directory links from selected text.
Falls back to Google search when no known pattern is matched.
...
class Config:
    """配置常量类"""
    # URL前缀
    # Replace these with your actual private URLs
    SR_PREFIX = 'https://support.example.com/sr?srNumber='
    DOC_PREFIX = 'https://kb.example.com/doc?id='
    BUG_PREFIX = 'https://bugdb.example.com/bug?rptno='
    JIRA_PREFIX = 'https://jira.example.com/browse/'
    PEOPLE_PREFIX = 'https://directory.example.com/'
    CLOUD_SR_PREFIX = 'https://cloud-sr.example.com/sr/edit?srNumber='
    SCHED_PREFIX = 'https://schedule.example.com/?email='
    ...
    PATTERNS = {
        ...
        'bug': r'(?:^|\s)\b\d{7,9}(?=\s|$)',
        'jira': r'(?:(?<=^)|(?<=\s)|(?<=\/))PROJ-\d{4,5}(?=\s|$)',
        'people': r"(?:^|\s)@[a-z]{3,20}(?=\s|$)"
    }
    
    # 允许的域名
    # Update with your allowed domains
    ALLOWED_DOMAINS = [
        'example.com',
        'google.com'
    ]
...
class LinuxInterface(PlatformInterface):
    ...
    def send_notification(self, message: str) -> None:
        """发送Linux通知"""
        subprocess.run(['notify-send', '-t', '1000', message], check=False)
...
class URLProcessor:
    ...
    def generate_urls(self, items: Dict[str, List[str]]) -> List[str]:
        ...
        for cmos in items.get('cmos', []):
            urls.append(f"{self.config.CLOUD_SR_PREFIX}{cmos.strip()}&action=EDIT_IN_TAB")
        ...
        for people in items.get('people', []):
            urls.append(f"{self.config.PEOPLE_PREFIX}{people.strip()}")
        
        # Email schedule URLs
        for email in items.get('email', []):
            sched_url = (f"{self.config.SCHED_PREFIX}"
                      f"{email.strip().upper()}"
                      f"&fromDate={self.date_key1}&toDate={self.date_key2}")
            urls.append(sched_url)
...
class OQOTool:
    ...
    def _setup_logging(self):
        """设置日志系统"""
        log_path = os.path.expanduser('~/.oqo/oqo.log')
